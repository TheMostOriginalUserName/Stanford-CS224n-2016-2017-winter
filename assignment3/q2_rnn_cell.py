...
class RNNCell(tf.contrib.rnn.RNNCell):#tf.nn.rnn_cell.RNNCell):  # https://github.com/tensorflow/tensorflow/issues/7664
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable('W_x',
                                  (self.input_size, self._state_size),
                                  initializer=tf.glorot_uniform_initializer())
                                  # ie tf.contrib.layers.xavier_initializer()
