...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) > max_length:
            ...
            _masking_ = [True] * max_length
        elif len(sentence) < max_length:
            delta = max_length - len(sentence)
            _masking_ = [True] * len(sentence) + [False] * delta
            sentence += [zero_vector] * delta
            labels += [zero_label] * delta
        ret.append((sentence, labels, _masking_))
        '''
        # new: doesn't change data. necessary. some data will be used for eval
        _masking_ = [True] * len(sentence)
        to_add = max(max_length - len(sentence), 0)
        ret.append(((sentence + [zero_vector] * to_add)[:max_length],
                    (labels   + [zero_label]  * to_add)[:max_length],
                    (_masking_+ [False]       * to_add)[:max_length]))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        tmp = tf.nn.embedding_lookup(emb, self.input_placeholder)
        embeddings = tf.reshape(tmp,
                                (-1,
                                 self.max_length,
    ...
    def add_prediction_op(self):
        ...
        Hint: You will find the function tf.pack (similar to np.asarray)
              useful to assemble a listvg of tensors into a larger tensor.
        ...
        # h at each step is a different tensor
        # only variables can be reused
        with tf.variable_scope('add_prediction_op') as varscope:
            pass            
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o, h = cell(x[:, time_step, :], h, varscope)
                varscope.reuse_variables()  # o, h is not reused
                od = tf.nn.dropout(o, dropout_rate) # od is not reused
                pred = tf.matmul(od, U) + b2
                # each step should generate a different pred
                preds.append(pred)
                ### END YOUR CODE
    ...
    def add_training_op(self, loss):
        ...
        self.pred_argmax = tf.argmax(self.pred, axis=2) # improved #
        ### END YOUR CODE
        return train_op
    ...
    def predict_on_batch(self, sess, inputs_batch, mask_batch):
        ...
        predictions = sess.run(self.pred_argmax, feed_dict=feed) # improved #
        return predictions
