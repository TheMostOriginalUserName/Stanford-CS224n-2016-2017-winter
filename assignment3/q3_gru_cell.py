...
class GRUCell(tf.contrib.rnn.RNNCell):#tf.nn.rnn_cell.RNNCell):  # https://github.com/tensorflow/tensorflow/issues/7664
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            Uz = tf.get_variable(name='U_z',
                                 shape=(self.input_size, self._state_size),
                                 initializer=tf.glorot_uniform_initializer())
            Wz = tf.get_variable(name='W_z',
                                 shape=(self._state_size, self._state_size),
                                 initializer=tf.orthogonal_initializer())
            bz = tf.get_variable(name='b_z',
                                 shape=(self._state_size,),
                                 initializer=tf.ones_initializer())
            Ur = tf.get_variable(name='U_r',
                                 shape=(self.input_size, self._state_size),
                                 initializer=tf.glorot_uniform_initializer())
            Wr = tf.get_variable(name='W_r',
                                 shape=(self._state_size, self._state_size),
                                 initializer=tf.orthogonal_initializer())
            br = tf.get_variable(name='b_r',
                                 shape=(self._state_size,))
            Uo = tf.get_variable(name='U_o',
                                 shape=(self.input_size, self._state_size),
                                 initializer=tf.glorot_uniform_initializer())
            ...
            bo = tf.get_variable(name='b_o',
                                 shape=(self._state_size,))
        zt = tf.nn.sigmoid(tf.matmul(inputs, Uz) + tf.matmul(state, Wz) + bz)
        rt = tf.nn.sigmoid(tf.matmul(inputs, Ur) + tf.matmul(state, Wr) + br)
        ot = tf.nn.tanh(tf.matmul(inputs, Uo) + rt * tf.matmul(state, Wo) + bo)
        new_state = zt * state + (1-zt) * ot
            ### END YOUR CODE ###
