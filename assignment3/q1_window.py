...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        start_words = [start,] * window_size
        end_words = [end,] * window_size
        start_labs = [4,] * window_size  # i guess..
        end_labs = [4,] * window_size
        sent = start_words + sentence + end_words
        labs = start_labs + labels + end_labs
        # for each center word
        for i in xrange(window_size, window_size + len(sentence)):
            # for each word around center, add to window
            win = []
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope('add_prediction_op'):
            W = tf.get_variable(
                        'W',
                        (self.config.n_window_features*self.config.embed_size,
    ...
    def add_training_op(self, loss):
        ...
        with tf.variable_scope('add_training_op'):
