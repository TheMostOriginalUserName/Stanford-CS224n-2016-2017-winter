...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self): 
        ...
        _, fin = tf.nn.dynamic_rnn(cell, x, dtype=tf.float32)
        preds = tf.nn.sigmoid(fin)
        ### END YOUR CODE
    ...
    def add_training_op(self, loss):
        ...
        if self.config.clip_gradients:
            '''
            can't believe tf.clip_by_global_norm has numerical issue:
                grad, self.grad_norm =\
                    tf.clip_by_global_norm(grad, self.config.max_grad_norm)
            sometimes gives grad_norm > max_grad_norm
            diagnosis code below
            my first workaround was to call it twice

            then decided to condition it. difficulty:
            grad, _ = zip(*optimizer.compute_gradients(loss)) # grad is a tuple
            grad, _ = tf.clip_by_global_norm                  # grad is a list
            had difficulty changing the latter grad to a tuple
            so changed the former to list
            '''

            def f1():
                return tf.clip_by_global_norm(grad, self.config.max_grad_norm)
            def f2():
                return grad, self.grad_norm
            grad, self.grad_norm =\
                tf.cond(self.grad_norm > tf.constant(self.config.max_grad_norm,
                                                     dtype=tf.float32),
                        f1, f2)

        '''diagnosis code'''
        #ass = tf.Assert(tf.less_equal(self.grad_norm,
        #                              self.config.max_grad_norm),
        #                [self.grad_norm,
        #                 tf.constant(self.config.max_grad_norm)])
        #ppp = tf.Print(self.grad_norm, [self.grad_norm])
        ## doesn't print in ipython
        train_op = optimizer.apply_gradients(zip(grad, var))#, ass]
        ### END YOUR CODE
